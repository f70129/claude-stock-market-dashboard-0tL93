"""Technical analysis: indicators + Cypher XABCD harmonic auto-detection."""
import numpy as np
import pandas as pd
...
#    C extends  XA  : 1.272 – 1.414  (from X origin)
#    D retraces XC  : 0.782  (±tol)  ← PRZ
#
#  Bullish : X(Lo)→A(Hi)→B(Lo)→C(Hi)→D(Lo/PRZ)  → 做多
#  Bearish : X(Hi)→A(Lo)→B(Hi)→C(Lo)→D(Hi/PRZ)  → 做空
...
def _score(b, c, d):
    be = abs(b - 0.500) / 0.500
    ce = abs(c - 1.341) / 1.341
    de = abs(d - 0.782) / 0.782
    return max(0.0, 100 - (be + ce + de) / 3 * 200)
...
def detect_cypher(df: pd.DataFrame,
    ...
    for i in range(len(all_p) - 3):
        ...
        if Xt=="L" and At=="H" and Bt=="L" and Ct=="H":
            ...
            XC = Cp - Xp
            prz = Xp + XC * 0.782
            if _within(Br, 0.382, 0.618, tol) and _within(Ce, 1.272, 1.414, tol):
                # search next Low for D
                cands = [(d,p,t) for d,p,t in all_p[i+4:] if t=="L" and d > Cd]
                if cands:
                    Dd,Dp,_ = cands[0]; Dr = _ret(Xp, Cp, Dp)
                    done = _within(Dr, 0.732, 0.832, tol)
                else:
                    Dd,Dp,Dr,done = Cd,prz,0.782,False
                results.append(CypherPattern(
                    "bullish",(Xd,Xp),(Ad,Ap),(Bd,Bp),(Cd,Cp),(Dd,Dp),
                    round(prz,4), round(prz*(1-tol),4), round(prz*(1+tol),4),
                    done, round(_score(Br,Ce,Dr),1),
                    round(Br,4), round(Ce,4), round(Dr,4),
                ))
        ...
        if Xt=="H" and At=="L" and Bt=="H" and Ct=="L":
            ...
            prz = Xp - XC * 0.782
            if _within(Br, 0.382, 0.618, tol) and _within(Ce, 1.272, 1.414, tol):
                cands = [(d,p,t) for d,p,t in all_p[i+4:] if t=="H" and d > Cd]
                if cands:
                    Dd,Dp,_ = cands[0]; Dr = _ret(Cp, Xp, Dp)
                    done = _within(Dr, 0.732, 0.832, tol)
                else:
                    Dd,Dp,Dr,done = Cd,prz,0.782,False
                results.append(CypherPattern(
    ...
    # De-duplicate: keep best quality per X-point per direction
    seen = {}
    for p in results:
...
def calc_target_price(df: pd.DataFrame) -> dict:
    if len(df) < 20: return {}
    df   = add_all_indicators(df); last = df.iloc[-1]
